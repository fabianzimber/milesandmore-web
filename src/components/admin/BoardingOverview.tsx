    ...
    <SASCard variant="glass" padding="none">
      ...
      <div className="px-5 py-3.5 border-b border-white/20 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Users size={15} className="text-aviation-blue" />
          <span className="text-sm font-semibold text-foreground">Passagiere</span>
        </div>
        <div className="flex items-center gap-3 text-xs">
          ...
          <div className="w-24 h-2 bg-white/[0.05] rounded-full overflow-hidden">
            <motion.div
              className="h-full rounded-full bg-gradient-to-r from-aviation-blue via-aviation-blue/70 to-cyan-400 relative overflow-hidden"
              initial={{ width: 0 }}
              animate={{ width: `${pct}%` }}
          ...
          <span className="text-foreground/40">{flight.aircraft_total_seats}</span>
        </div>
      </div>
      ...
      <div className="max-h-64 overflow-y-auto sm:max-h-96">
        <AnimatePresence>
          {participants.map((p, i) => (
            <motion.div
              ...
              className="flex items-center justify-between px-5 py-2.5 border-b border-white/[0.04] hover:bg-white/[0.04] transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-foreground/80 truncate max-w-[120px] sm:max-w-none">{p.user_name}</span>
              </div>

              {p.seat ? (
                <span className="text-[11px] font-mono font-bold bg-gradient-to-r from-aviation-blue to-aviation-blue/70 text-white px-2.5 py-1 rounded-md shadow-sm">
                  {p.seat}
                </span>
              ) : (
        ...
        {participants.length === 0 && (
          ...
          >
            ...
            <p className="text-sm text-foreground/40 mt-3">Warte auf Passagiere...</p>
          </motion.div>
        )}
      </div>
    </SASCard>
